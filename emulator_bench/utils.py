...
DEFAULT_TS_LOG_ROOT = DEFAULT_RUNS_ROOT / "ts_logs"


def resolve_path(path: str | Path, *, base: Path = BASELINE_ROOT) -> Path:
    resolved = Path(path).expanduser()
    if not resolved.is_absolute():
        baseline_candidate = base / resolved
        project_candidate = PROJECT_ROOT / resolved
        if not baseline_candidate.exists() and project_candidate.exists():
            resolved = project_candidate
        else:
            resolved = baseline_candidate
...
def choose_clipzyme_precision(requested: str) -> str:
    if requested == "auto":
        try:
            import torch
...
def submit_ts_job(
    ...
    *,
    ...
    if not log_path.is_absolute():
        log_path = DEFAULT_TS_LOG_ROOT / log_path
    ensure_dir(log_path.parent)
    args = [spooler, "-L", label, "-O", log_path.name]
    if gpus is not None:
        if gpus < 0:
    ...
    redirected_command = f"{command} > {shell_join([log_path])} 2>&1"
    args.extend(["bash", "-lc", redirected_command])
    ...
    with tqdm(total=len(remaining), desc="ts jobs", unit="job") as progress:
        while remaining:
            ...
            for job_id in sorted(remaining):
                ...
                elif "failed" in lowered or "error" in lowered or "skipped" in lowered:
...
def find_checkpoint(checkpoint_dir: str | Path) -> Path:
    ...
    last_paths = sorted(checkpoint_dir.rglob("last.ckpt"), key=lambda path: path.stat().st_mtime)
    if last_paths:
        return last_paths[-1]
    checkpoints = sorted(
        checkpoint_dir.rglob("*.ckpt"),
